...
function* changePage(action) {
	try {
		...
		if (page.isFirstLoad) {
			yield put({ type: 'ROUTER_FIRST_INIT', payload: { isFirstLoad: false } })
			yield put({ type: 'ROUTER_LOAD_PROGRESSING', payload: { loadInProgress: false } })
			return
		}
		yield put({ type: 'ROUTER_LOAD_PROGRESSING', payload: { loadInProgress: true } })
